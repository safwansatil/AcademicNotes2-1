...
![instructionFormat](./img/instructionFormat.png)
...
### The Analogy: Following a Cooking Recipe
Imagine the CPU is a chef following recipes (instructions) from a cookbook (memory).

---

**Step 1: Instruction Address Calculation (IAC)**
"Which recipe should I do next?"

Chef's thought: "Hmm, last I did Recipe #5. Now I should do Recipe #6."

- CPU looks at Program Counter (PC) to see what instruction comes next

- Usually just PC + 1 (go to next instruction)

- Unless it's a jump/branch, then calculate a new address

Example: PC = 0x100 → Next instruction is at 0x104

**Step 2: Instruction Fetch (IF)**
"Go get that recipe card"

- Chef's action: Walks to the cookbook, finds page 6, takes the recipe card.

- CPU puts the address (from Step 1) into MAR

- Reads memory at that address into MBR

- Moves it from MBR to Instruction Register (IR)

Example: Fetch ADD R1, R2, R3 from memory location 0x104

**Step 3: Instruction Operation Decoding (IOD)**
"What does this recipe want me to do?"

- Chef's thought: Reads the card: "Ah, this says 'Add sugar and flour'"

- CPU looks at the instruction in IR

- Figures out: "This is an ADD operation"

- Identifies: "It needs R2 and R3 as inputs, R1 as output"

Example: Decode ADD R1, R2, R3 → Operation = ADD, Inputs = R2 & R3, Output = R1

**Step 4: Operand Address Calculation (OAC)**
"Where are the ingredients?"

- Chef's thought: "Sugar is in Pantry Shelf B4. Flour is in Shelf A2."

- For LOAD/STORE instructions: Calculate where in memory the data is

- For register operations: This step is skipped (data is already in registers)

- The result is a memory address (if needed)

Example: For LOAD R1, 0x200 → Calculate that operand is at address 0x200

**Step 5: Operand Fetch (OF)**
"Go get the ingredients"

- Chef's action: Goes to pantry, gets sugar from B4 and flour from A2.

- CPU puts the address (from Step 4) into MAR

- Reads the actual data from memory into MBR

- For register operations: Just reads from registers directly
Example: Fetch value 5 from memory address 0x200 (for R2)
Fetch value 3 from memory address 0x204 (for R3)

**Step 6: Data Operation (DO)**
"Do the actual cooking"

- Chef's action: Actually mixes the sugar and flour together.

- The ALU (Arithmetic Logic Unit) performs the operation

- Could be ADD, SUBTRACT, MULTIPLY, COMPARE, etc.

Example: ALU adds 5 + 3 → Gets result 8

**Step 7: Operand Store (OS)**
"Put the result where it belongs"

- Chef's action: Puts the mixed batter into a bowl (register) or into the fridge (memory).

- Store the result from Step 6

- Could be into a register (fast) or back to memory (slow)

Example: Store result 8 into register R1
...
![interruptionMechanism](./img/interruptionMechanism.png)
![interruptionMechanismContd](./img/interruptionMechanismContd.png)
...
![busProblems](./img/busProblems.png)
...
**_QPI's 4 layers_**: physical, link, routing, protocol (reliable delivery, routing decisions, coherence rules).
